...
def load_sales_records():
    if os.path.exists(SALES_FILE):
        ...
        return pd.DataFrame(columns=['时间', '商品名称', '销售数量', '销售单价', '销售额'])
# ========== 侧边栏配置 ==========
# 每次加载页面都重新读保质期配置，保证实时同步
...
with st.sidebar:
    ...
    with data_tab3:
        ...
        if existing_df is None or len(existing_df) == 0 or '商品名称' not in existing_df.columns:
            st.info("暂无库存数据，先添加商品，或检查上传文件列名是否包含「商品名称」")
        else:
            # 补全需要的列，避免KeyError
            if '单位' not in existing_df.columns:
                existing_df['单位'] = DEFAULT_UNIT
            if '库存数量' not in existing_df.columns:
                existing_df['库存数量'] = 0
            # 清洗库存数量为数值
            existing_df['库存数量'] = pd.to_numeric(existing_df['库存数量'], errors='coerce').fillna(0).clip(lower=0)
            all_products = existing_df['商品名称'].dropna().astype(str).str.strip().unique().tolist()
            if not all_products:
                st.info("暂无有效商品数据")
            else:
                with st.form("sales_form", clear_on_submit=True):
                    ...
                    max_qty = float(p_info['库存数量']) if p_info['库存数量']>0 else 0.0
                    sell_qty = col_s1.number_input("出库数量", min_value=0.1, step=0.1, max_value=max(max_qty,0.1))
                    sell_price = col_s2.number_input("销售单价(元)", min_value=0.0, step=0.1, value=0.0)
                    st.caption(f"当前库存：{p_info['库存数量']:.0f}{p_info['单位']}")
                    submit_sell = st.form_submit_button("确认出库并扣减库存", use_container_width=True, type="primary")
                    if submit_sell and sell_qty <= max_qty:
                        # 扣减库存
                        ...
                        st.success(f"✅ 出库成功：{sell_qty}{p_info['单位']} {sell_p}，销售额{sales_amount:.1f}元")
                        st.cache_data.clear()
                        st.rerun()
    # 批量盘点功能
    with data_tab4:
        ...
        existing_df, load_status = load_latest_inventory()
        if existing_df is None or len(existing_df) == 0 or '商品名称' not in existing_df.columns:
            st.info("暂无库存数据，或上传文件列名不正确")
        else:
            # 补全需要的列
            if '品类' not in existing_df.columns:
                existing_df['品类'] = '未分类'
            if '单位' not in existing_df.columns:
                existing_df['单位'] = DEFAULT_UNIT
            if '库存数量' not in existing_df.columns:
            ...
            edited_df = st.data_editor(
                existing_df[['商品名称', '品类', '库存数量', '单位']].dropna(subset=['商品名称']),
    ...
    with data_tab6:
        existing_df, load_status = load_latest_inventory()
        if existing_df is None or len(existing_df) == 0 or '商品名称' not in existing_df.columns:
            st.info("暂无库存数据，先上传或添加商品，或检查上传文件列名")
        else:
            # 补全列避免报错
            if '单位' not in existing_df.columns:
                existing_df['单位'] = DEFAULT_UNIT
            ...
            if not all_products:
                ...
                if selected_p:
                    ...
                    current_stock = int(p_data['库存数量']) if not pd.isna(p_data['库存数量']) else 0
                    new_stock = st.number_input("修改库存数量", min_value=0, value=current_stock, step=1)
...
# 第二行：经营数据
st.markdown(f"<h4 style='text-align:center;color:{score_color};'>今日销售额：{today_sales:.1f}元</h4>", unsafe_allow_html=True)
st.divider()
# 临期预警区域（用筛选后的数据）
